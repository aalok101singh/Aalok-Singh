...
export default function ShortcutsModal(): JSX.Element {
  ...
  useEffect(() => {
    ...
    window.addEventListener('caregrid:shortcuts-dismiss', dismiss)
    return () => {
      window.removeEventListener('caregrid:shortcuts', on)
      window.removeEventListener('caregrid:shortcuts-dismiss', dismiss)
    }
  }, [])
  if (!open) return <></>
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-ink/40" onClick={() => setOpen(false)}>
      <div role="dialog" aria-modal="true" className="w-80 rounded-card border border-border bg-surface p-4 shadow-card" onClick={(e) => e.stopPropagation()}>
